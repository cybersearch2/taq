...
public class Personalities
{
    ...
    {
        QueryProgram queryProgram = queryProgramParser.loadScript("basic_lists/personalities.taq");
        // Create QueryParams object for Global scope and query "person_search"
    ...
    static public List<Profile> doSearch(String name) {
        ...
        while(iterator.hasNext())
        {
			...
			ItemList<String> itemList = (ItemList<String>) axiom.getValueByName("traits");
        	StringBuilder builder = new StringBuilder();
        	itemList.forEach(trait -> {
        		if (builder.length() > 0)
        			builder.append(", ");
        		builder.append(trait);
        	});
        	profile.traits = builder.toString();
