...
public class MultiCurrencyTest
{
    ...
    public void testMultiCurrency() throws Exception
    {
    	...
    	args.add("multi-currency");
        Taq taq = new Taq(args);
		taq.findFile();
		taq.compile();
		Result result = taq.getQueryProgram().executeQuery("price_query");
        Iterator<Axiom> priceIterator = result.axiomIterator("price_query");
        File testFile = ResourceHelper.getResourceFile("operations/multi-currency.txt");
        final BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(testFile), "UTF-8"));
        assertThat(priceIterator.hasNext()).isTrue();
