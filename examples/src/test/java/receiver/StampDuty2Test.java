...
public class StampDuty2Test
{
    ...
    public void testStampDuty2() throws Exception
    {
    	...
    	args.add("stamp-duty2");
        Taq taq = new Taq(args);
        List<String> captureList = taq.getCaptureList();
		taq.findFile();
		taq.compile();
        File testFile = ResourceHelper.getResourceFile("receiver/stamp-duty2.txt");
        final BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(testFile), "UTF-8"));
        taq.execute();
        Iterator<String> dutyIterator = captureList.iterator();
        while (dutyIterator.hasNext())
            checkSolution(reader, dutyIterator.next());
    }
