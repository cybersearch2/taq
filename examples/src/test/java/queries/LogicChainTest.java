...
public class LogicChainTest
{
    ...
    public void testLogicChain() throws Exception
    {
    	...
    	args.add("logic-chain");
        Taq taq = new Taq(args);
        List<String> captureList = taq.getCaptureList();
		taq.findFile();
		taq.compile();
		taq.execute();
        Iterator<String> deliveryIterator = captureList.iterator();
        File testFile = ResourceHelper.getResourceFile("queries/logic-chain.txt");
        final BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(testFile), "UTF-8"));
        assertThat(deliveryIterator.hasNext()).isTrue();
